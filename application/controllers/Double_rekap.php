...
class Double_rekap extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('rekap/double_rekap', $data);
    ...
    public function showRekap()
    {
        ...
        //looping db if role = 1
        $brandArray = array();
        foreach ($resultDb as $dbb) {
            $dbArray[] = $dbb->db;
        }
        ...
        $brandArray = array();
        for ($d = 0; $d < count($dbArray); $d++) {
            $resultBrand = $this->database->getData($queryBrand, $dbArray[$d])->result();

            foreach ($resultBrand as $br) {

                $brandName[] = $br->slug_franchise;
                $dbName[] = $dbArray[$d];
                $brandId[] = $br->id_franchise;

                $queryTotal = "SELECT id_visit, COUNT(id_visit) AS jumlah, brand
                          FROM visit
                          WHERE brand = $br->id_franchise
                          AND pt = $idPt
                          AND tanggal_input >= '$tanggalAwal'
                          AND tanggal_input <= '$tanggalAkhir'";
                ...
                if ($role == 1) {
                    $queryRespon = "SELECT nama, brand, id_visit, kategori
                    FROM visit
                    WHERE brand = $br->id_franchise
                    AND kategori = 3
                    AND pt = $idPt
                ...
                $resultRespon = $this->db->query($queryRespon);

                $respon[] = $resultRespon->num_rows();

                if ($role == 1) {
                    $queryNorespon = "SELECT nama, brand
                    FROM visit
                    WHERE brand = $br->id_franchise
                    AND pt = $idPt
                    AND kategori = 2
                ...
                $resultNorespon = $this->db->query($queryNorespon);

                $noRespon[] = $resultNorespon->num_rows();

                if ($role == 1) {
                    $queryAnak = "SELECT nama, brand
                    FROM visit
                    WHERE brand = $br->id_franchise
                    AND pt = $idPt
                    AND kategori = 1
                ...
                $resultAnak = $this->db->query($queryAnak);

                $anak[] = $resultAnak->num_rows();
            }
            print_r($resultBrand);
        }
        print_r($brandName);
        ...
        for ($i = 0; $i < count($respon); $i++) {
            ...
            if ($kategori == 4) {
                ...
                $total[] = $respon[$i] + $noRespon[$i] + $anak[$i];
            } else if ($kategori == 3) {
        ...
        if ($role == 5) {
            $queryMaster = "SELECT id_visit, kategori
                        FROM visit
                        WHERE tanggal_input >= '$tanggalAwal'
                        AND tanggal_input <= '$tanggalAkhir'";
        } else {
            $queryMaster = "SELECT id_visit, kategori
                        FROM visit
